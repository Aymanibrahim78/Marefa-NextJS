...
async function getData() {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}blogs?populate=*`, { next: { revalidate: 100 } });
...
<Stack direction={"row"} gap={2} flexWrap={"wrap"} my={4} >
  {data.data.reverse().map((item)=>{
    return(
      ...
      <CardBlogInBlogPage key={item.id} title={item.attributes.title} description={item.attributes.description}
